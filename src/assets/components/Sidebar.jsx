    ...
    <div className="fixed h-full">
      ...
      <button
        ...
        className={`fixed top-[70px] left-[42px] w-8 h-8 rounded-full flex items-center justify-center cursor-pointer transition-all z-20 shadow-md
          ${isOpen ? "translate-x-[162px]" : ""}
          bg-gray-200 dark:bg-gray-700 bg-opacity-90 backdrop-blur-sm`}
      >
      ...
      <div
        className={`fixed z-10 h-full transition-all duration-100 ease-in-out overflow-y-auto overflow-x-hidden pt-5
      ...
      >
        ...
        <div className="flex justify-center items-center pb-[60px]">
          <div
            className={`flex justify-center items-center w-[35px] cursor-pointer transition-all duration-300 ease
              ${isOpen ? "scale-75" : "scale-150"}`}
          >
            <img 
              src="/logo.png"  // Reemplazar logo
              className="w-full" 
              alt="Logo" 
            />
          </div>
        ...
        {LinksArray.map(({ icon, label, to }) => (
          <div
            className={`my-1 transition-all duration-300 ease-in-out relative px-1
              ${isOpen ? "px-0" : "px-[5%]"}
